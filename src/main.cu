...
int main( int argc, char** argv) 
{
	...
	DoLog("** load matrice...");
	int matrixSize = sizeof(float)*(M_ROW_COUNT*M_COL_COUNT + 1);

	float *d_matrix1 = NULL;

	checkCudaErrors(cudaMalloc((void**)&d_matrix1, matrixSize));
	checkCudaErrors(cudaMemcpy(d_matrix1, matrix1, matrixSize, cudaMemcpyHostToDevice));

	float *d_matrix2 = NULL;
	checkCudaErrors(cudaMalloc((void**)&d_matrix2, matrixSize));
	checkCudaErrors(cudaMemcpy(d_matrix2, matrix2, matrixSize, cudaMemcpyHostToDevice));
	...
	free(matrix2);
	free(tmpMatrix2);
	checkCudaErrors(cudaFree(d_matrix1));
	checkCudaErrors(cudaFree(d_matrix2));
